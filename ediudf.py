from udf_spec import *
import os
import sys
from lookups import *
import tester
from errorhan import errorhandle, finalerrors
from copy import copy



error_file = open("errorrpt.txt", "w")
...
with open('C:\\transfer\\TrueEDI\\'+fname) as file:  
        listy1 = file.read()
# udf_award = udf[14:]
...
for segment in awards:
    ...
    for spec in udf_award:
        if len(spec)>1 and spec[1]=='NTE' and value!='864':
        ...
        if len(spec)>1 and (spec[1]=='SDQ' or spec[1]=='TD1') and value!='' and value!='AQ':
...
if sys_not!=[]:
    ...
    for notice in sys_not:
        mit_count = len([line[0] for line in notice if line[0]=='MIT'])
        sys_not_udfs = sys_not_udf + (sys_not_udf[-5:]*(mit_count-1))
        s3_count=-1
        for spec in sys_not_udfs:
            if spec[0] == '\nS3':
                s3_count+=1
            if len(spec)>3:
                length = spec[3]
                if spec[2]=='' and len(spec)<6:
                    value = nocode_lookup(spec[1], notice,pos=spec[4], skip=s3_count)
                ...
                if len(spec)>6:
                    try:
                        value = cond_lookup(spec[1],spec[4],spec[5],notice,[spec[6],spec[7]],0)
                    except:
                        try:
                            value =cond_lookup(spec[1],spec[4],spec[5],header_segment,[spec[6],spec[7]],0)
                        except:
                            value = ''
                if spec[1]=='MIT':
                    value = nocode_lookup(spec[1], notice,pos=spec[4], skip=s3_count)
...
print('UDF print attached. Can also be found as the filename', fname, ' ".DAT" in the local drive:\n')
udf_string=udf_string+'\nÿ'
# print(udf_string)

# # # SEND IT TO UDF FILE FORMAT (TAKE AWAY V\TEST) # # #

text_file = open(fname[:-3]+'DAT', "w")
...
# tester.test(udf_string,fname)
tester.test(udf_string,'C:\Transfer\TrueDAT\\'+fname[:-3]+'DAT')
# tester.test(udf_string,'Z:\TrueDAT\\'+fname[:-3]+'DAT')


# tester.contenttest(udf_string,'C:\Transfer\TrueDAT\\'+fname[:-3]+'DAT')

# print('Sent ', 'Z:\TrueDAT\\'+fname[:-3]+'DAT', 'to Test' )
# print('Sent ', 'C:\Transfer\TrueDAT\\'+fname[:-3]+'DAT', 'to Test' )
