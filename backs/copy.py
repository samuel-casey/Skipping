...
# with open('udf_specs.txt') as file:  
#     udf = file.read() 
# print(udf)
udf_header = udf[:14]
udf_award = udf[14:]
...
def segment_it(listy):
    ...
    for line in listy:
        ...
        line_no+=1
    return segmented
def find_value(element, code, pos, listy, LIN=False):
    ...
    for line in listy:
        ...
        if LIN:
            for string in line:
                if element == found_element and string==code:
                    value = line[line.index(string)+1]
                    print(value, "YES")
                else:
                    print(found_element, string)
    return value 
def make_udf(udf):
    udf_string = ''
    for element in udf:
...
for segment in listy[1:]:
    for spec in udf_award:
        if len(spec)>1:
            ...
            length = len(spec[0]) 
        if len(spec) == 6:
            value = find_value(spec[1],spec[2],spec[4],segment, LIN=True)
        if len(spec) == 7:
            # print(value)
            value = value[spec[5]:spec[6]]
